...
const generateGalaxySpiral = (count) => {
  const positions = new Float32Array(count * 3);
...
const GalacticCore = ({ progress }) => {
  const ref = useRef();
  const [coreData] = useState(() => {
    const count = 2000;
    const positions = new Float32Array(count * 3);
    const colors = new Float32Array(count * 3);
  ...
  useFrame((state, delta) => {
    if (ref.current) {
      ...
      ref.current.material.opacity = Math.max(0, (1 - progress * 1.5) * 0.9);
...
const MilkyWayGalaxy = ({ progress }) => {
  const ref = useRef();
  const [galaxyData] = useState(() => generateGalaxySpiral(15000)); // Increased density
  ...
  useFrame((state, delta) => {
    if (ref.current) {
      ...
      // Fade out as we zoom in
      ref.current.material.opacity = Math.max(0, 1 - progress * 1.2);
...
const StarBackground = ({ progress }) => {
  const ref = useRef();
  const [sphere] = useState(() => generateSphere(8000, 1.5)); // Increased star count
  ...
  useFrame((state, delta) => {
    if (ref.current) {
      ...
      ref.current.material.opacity = Math.min(1, progress * 1.5);
    }
  });
...
const CameraAnimation = ({ progress, setProgress, onAnimationComplete }) => {
...
const StarCanvas = ({ onAnimationComplete }) => {
